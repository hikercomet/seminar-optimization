...
# プロジェクトのルートは C:/Users/hiker/seminar_optimization である。
script_dir = os.path.dirname(os.path.abspath(__file__)) # C:/Users/hiker/seminar_optimization/seminar_optimization/seminar_optimization
# project_root は、Pythonパッケージ 'seminar_optimization' の一つ上のディレクトリを指すように調整
# この場合、C:/Users/hiker/seminar_optimization/seminar_optimization がパッケージの親ディレクトリ
project_root = os.path.dirname(os.path.dirname(script_dir)) # 2つ上のディレクトリに移動
...
try:
    ...
    # OptimizationResult を utils から明示的にインポート
    from utils import OptimizationResult
except ImportError as e:
    messagebox.showerror("エラー", f"最適化サービスモジュールのインポートに失敗しました: {e}\nPythonパスを確認してください。")
    sys.exit(1)
...
class SeminarGUI:
    ...
    def _initialize_defaults(self):
        ...
        default_data_dir = self.optimization_config.get('data_directory', 'data')
    ...
    def _browse_file(self, var: tk.StringVar, file_type: str):
        ...
        initial_dir = os.path.dirname(var.get()) if var.get() else './data'
